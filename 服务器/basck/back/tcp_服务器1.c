...
char *admin_all[2] = {NULL, NULL};                //保存登录用户工号、姓名
...
unsigned char *tips[] = {
    ...
    "输入有误，请重新输入：","查询结果失败","员工信息查询完成","操作失败"};
/************************************************************************************************************/
...
void *data_interaction(void *arg)
{
    ...
    char nbuf[128];
    int falg = 1;
    char *admin[2] = {NULL, NULL};  //保存当前线程下用户的工号、姓名
    ...
    while (buf[0])
    {
        ...
        while (buf[1])
        {
            ...
            if (strlen(nbuf) == 1)
            {
                ...
                case '1': //管理员登录;
                    usertypoe = 1;
                    sqlite3_long(db, sfd, usertypoe);      //调用登录函数
                    sprintf(admin[0], "%s", admin_all[0]); //保存工号
                    sprintf(admin[1], "%s", admin_all[1]); //保存用户名
                    break;
                case '2': //员工登录
                    usertypoe = 0;
    ...
    printf("客户端用户%s>>>%s:%d\n", admin[1], inet_ntoa(prd.cin.sin_addr), ntohs(prd.newfd));
    printf("已断开连接\n");
    close(sfd);
    sqlite3_close(db);
    pthread_exit(NULL);
...
int sqlite3_long(sqlite3 *db, int fsd, int usertypoe)
{
    ...
    if(recv(fsd,&st.new_buf[1][0],128,0)==0)
        return -1;
    send(fsd, tips[2], strlen(tips[2]), 0);
    sprintf(admin_all[0], "%s", &st.new_buf[0][0]); //保存登录用户工号信息
    sprintf(admin_all[1], "%s", &st.new_buf[1][0]); //保存登录用户姓名信息
    return 0;
}
...
//添加信息
int sqlite3_add(sqlite3 *db, int fsd, char *name[2])
{
    char buf[128] = {"\0"};
    char out;
    int falg = 1;
    ...
    do
    {
        ...
        time_cat = localtime(&t); //转换时间
        BUF_ZERO(buf);
        sprintf(buf, "insert into stu2 values(\"%s\",\"管理员%s\",\"添加了%s的员工\",\"%d月  %d日  %02d:%02d\")", name[0], name[1], \
                &st.new_buf[1][0], time_cat->tm_mon + 1, time_cat->tm_mday,
                time_cat->tm_hour, time_cat->tm_min);
        if (sqlite3_exec(db, buf, NULL, NULL, &errmsg) != SQLITE_OK)
        {
...
//删除信息
int sqlite3_del(sqlite3 *db, int fsd, char *name[2])
{
    char *errmsg = NULL;
    char buf[10] = {"\0"};
    char ubuf[128] = {"\0"};
    ...
    time_cat = localtime(&t); //转换时间
    BUF_ZERO(ubuf);
    sprintf(ubuf, "insert into stu2 values(\"%s\",\"管理员\"%s\"\",\"%s%s的员工\",\"%d月  %d日  %02d:%02d\")", name[0], name[1], tips[25], buf, time_cat->tm_mon + 1, time_cat->tm_mday,
            time_cat->tm_hour, time_cat->tm_min);
    if (sqlite3_exec(db, buf, NULL, NULL, &errmsg) != SQLITE_OK)
    {
        send(fsd,tips[38],strlen(tips[38]),0);
...
//修改信息
int sqlite3_revise(sqlite3 *db, int fsd, char *name[2], int usertypoe, char falg)
{
    /*指令码
    char *menu[] = {"姓名", "年龄", "住址", "电话", "职位", "工资", "入职时间", "评级", "密码"};
    char *menu_two[] = {"住址","电话","密码"};
    ...
    else if (usertypoe == 0)
    {
        ...
        if (recv(fsd, &buf[1][0], 128, 0) == 0)
            return -1;
        sprintf(nbuf, "update stu1 set \"%s\"=\"%s\" where 工号=\"%s\"", menu_two[falg], &buf[1][0], name[0]);
        if (sqlite3_exec(db, nbuf, NULL, NULL, &errmsg) != SQLITE_OK)
        {
            send(fsd,tips[38],strlen(tips[38]),0);
            fprintf(stderr, "%s:__%d__,sqlite3_exet:%s\n", __FILE__, __LINE__, errmsg);
    ...
    sprintf(nbuf, "insert into stu2 values(\"%s\",\"%s\",\"修改%s为%s\",\"%d月  %d日  %02d:%02d\")", name[0], name[1],p,\
...
//查找用户信息
int sqlite3_lookup(sqlite3 *db, int fsd, char *name[2], int usertypoe, int falg)
{
    int index = 0;
    char sql[128] = {"\0"};
    char **result = NULL;
    ...
    {
        //员工查找程序
        sprintf(sql,"select * from stu1 where 姓名=\"%s\"",name[0]);
        //管理员查找程序
        if (sqlite3_get_table(db, sql, &result, &row, &column, &errmsg) != SQLITE_OK)
        {
            send(fsd,tips[36],strlen(tips[36]),0);
    ...
    if(row&&column)
    {
        ...
        for(int i=0;i<=row;i++){
            ...
            result[(index)],9,result[(index+1)],9,result[(index+2)],9,result[(index+3)],9,result[(index+4)],9,result[(index+5)],9\
            ,result[(index+6)],9,result[(index+7)],9,result[(index+8)],9,result[(index+9)],9,result[(index+10)],9\
            ,result[(index+11)]);
            send(fsd,buf,sizeof(buf),0);
            BUF_ZERO(buf);
            index = index+12;
            ...
            //历史记录处理
            for(int i=0;i<=row;i++){
            sprintf(buf,"%s%c,%s%c,%s%c,%s",result[(index)],9,result[(index+1)],9,\
                result[(index+2)],9,result[(index+3)]);
            send(fsd,buf,sizeof(buf),0);
            BUF_ZERO(buf);
            index = index+4;
            }
