...
int sqlite3_long(sqlite3 *db, int fsd, int usertypoe)
{
    ...
    int falg = 1;
    BUF_ZEROS(&st,1536);
    //send(fsd,tips[18],strlen(tips[18]),0);
    //管理员判断
    if (usertypoe == 1)
    {
        //是否添加管理员
...
int sqlite3_revise(sqlite3 *db, int fsd, struct ad_data id, int usertypoe, char falg)
{
    ...
    if (usertypoe == 1)
    {
        ...
        return -1;
        send(fsd, tips[(menu_int[falg-1])], strlen(tips[(menu_int[falg-1])]), 0);
    ...
    {
        //员工修改程序
        send(fsd, tips[(menu_two_int[falg-1])], strlen(tips[(menu_two_int[falg-1])]), 0);
        if (recv(fsd, &buf[1][0], 128, 0) == 0)
            return -1;
